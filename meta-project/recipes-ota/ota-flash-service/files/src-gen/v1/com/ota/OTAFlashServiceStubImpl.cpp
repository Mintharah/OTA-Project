...
void OTAFlashServiceStubImpl::sendChunk(
  ...
  // fire progress event so Qt UI updates
  fireFlashProgressEvent(transferId, FlashState::RECEIVING,
                         flashProgressPercent_, OTAErrorCode::SUCCESS);
...
void OTAFlashServiceStubImpl::finalizeTransfer(
  ...
  // fire COMPLETE event FIRST so Qt UI shows success GIF
  fireFlashProgressEvent(transferId, FlashState::COMPLETE, 100,
                         OTAErrorCode::SUCCESS);

  reply(OTAErrorCode::SUCCESS);

  // reboot after 5 seconds in a detached thread
  std::thread([]() {
